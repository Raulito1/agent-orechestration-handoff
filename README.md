...
│                               └── build_prompt() assembles skill files + task
│
├── agents/
...
│   └── contexts/               Stack-specific system prompts for Claude (feature cmd)
│
├── .roo/skills/                Canonical skill files — loaded by rpi commands
│   ├── SKILL-rpi-research.md   Rules + output format for Research phase
│   ├── SKILL-rpi-plan.md       Rules + PLAN.md section spec for Plan phase
│   ├── SKILL-rpi-implement.md  Rules + BLOCKED/✓/IMPLEMENT DONE signals
│   ├── SKILL-fastapi.md        FastAPI conventions (RBAC pipeline, CTE, SQL-on-disk)
│   ├── SKILL-java-spring.md    Spring Boot conventions (reactive rules, MapStruct, Liquibase)
│   └── SKILL-react.md          React conventions (RTK Query, UIX primitives, shadcn/ui)
│
...
The `.roo/skills/` directory is the canonical source of conventions for AI-assisted coding. The `config/contexts/` files used by the `feature` command reference these skills but are kept separate so the existing `feature` command is unaffected.
...
If your new project uses a stack not yet covered (e.g., `node-express`):

1. Create `.roo/skills/SKILL-node-express.md` following the same structure as the existing skill files.
2. Add an entry to `_STACK_SKILL` in [orchestrators/rpi_orchestrator.py](agent-orchestrator/orchestrators/rpi_orchestrator.py):
   ```python
   _STACK_SKILL: dict[str, str] = {
       ...
       "node-express": "SKILL-node-express.md",
   }
   ```
3. Use `stack: node-express` in your `projects.yaml` repo entry.
...
├── .roo/
│   └── skills/
│       ├── SKILL-rpi-research.md
│       ├── SKILL-rpi-plan.md
│       ├── SKILL-rpi-implement.md
│       ├── SKILL-fastapi.md
│       ├── SKILL-java-spring.md
│       └── SKILL-react.md
├── tasks/
│   ├── README.md
│   ├── status.json              (auto-generated)
│   ├── <repo-name>.md           (task brief — you write this)
│   ├── <repo-name>-RESEARCH.md  (auto-generated by rpi research)
│   └── <repo-name>-PLAN.md      (auto-generated by rpi plan)
