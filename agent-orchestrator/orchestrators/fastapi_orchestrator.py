"""FastAPI Orchestrator — sequences FastAPI agent → React agent for feature generation."""
...
from agents.fastapi_agent import BackendGenerationResult, FastAPIAgent
...
def _feature_slug(description: str) -> str:
    """First 5 words of description, hyphenated, lowercase."""
...
def _component_name(description: str) -> str:
    """PascalCase name from first 3 words."""
    words = description.split()[:3]
    return "".join(w.capitalize() for w in words) + "Component"


import re


class FastAPIOrchestrator(BaseOrchestrator):
    """Orchestrates FastAPI + React feature generation."""
    ...
    def run(
        ...
        # Load context files
        fastapi_context = self._load_context_safe(app_config.get("contexts", {}).get("api", ""))
        react_context = self._load_context_safe(app_config.get("contexts", {}).get("web", ""))

        # Step 1: FastAPI agent
        if progress_callback:
            progress_callback("fastapi_start")

        fastapi_agent = FastAPIAgent(
            client=self.get_anthropic_client(),
            system_prompt=fastapi_context,
        )
        backend_result: BackendGenerationResult = fastapi_agent.generate(description, app_name)

        if progress_callback:
            progress_callback("fastapi_done", backend_result.files)
        ...
        _write_output(out_dir, backend_result, react_result, description, slug)
...
def _write_output(
    ...
    for file_type, code in backend.files.items():
        filename = _BACKEND_FILENAMES.get(file_type, f"{file_type}.py")
